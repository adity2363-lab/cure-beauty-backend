...
router.post("/", (req, res) => {
  ...
  const priced = items.map((it) => {
    ...
    return { ...it, price: product.price, name: product.name };
  });
  ...
  const address = addressId ? db.prepare("SELECT * FROM addresses WHERE id = ?").get(addressId) : null;
  notifyNewOrder({ orderId, customerName: user.name, customerPhone: user.phone, total, items: priced, address });
